    ...
    switch (response) {
        ...
        case 'add an employee':
            inquirer.prompt([
                ...
                },
                {
                    type: 'number',
                    name: 'roleId',
                    message: "What is the employee's role_id? (What is the employee's new role_id? (You can run the view all role id's by selecting view all roles at the main menu)",
                    validate: roleInput => {
        ...
        case 'update an employee role':
            ...
            const queryCallback = function(empArray) {
                inquirer.prompt([
                    {
                        type: 'checkbox',
                        name: 'empToPromote',
                        message: "Which employee's role is changing? (Please only choose one)",
                        choices: empArray,
                        validate: empInput => {
                            if (empInput[1]) {
                                return false;
                ...
                .then((empToChange) => {
                    empId = (empArray.indexOf((empToChange.empToPromote[0]).toString())) + 1;
                    ...
                    inquirer.prompt(
                        {
                            type: 'number',
                            name: 'roleId',
                            message: "What is the employee's new role_id? (You can run the view all role id's by selecting view all roles at the main menu)",
                            validate: roleInput => {
                                if (typeof roleInput === 'number') {
                                    return true;
                                } else {
                        ...
                        db.query(`UPDATE employee SET role_id = ${newRoleId.roleId} WHERE id = ${empId};`, (err, rows) => {
                            if (err) {
                                console.log(err)
                            }
                        });
            ...
            const queryEmps = function() {
                db.query(`SELECT concat(employee.first_name, ' ', employee.last_name) as employees
    ...
    inquirer.prompt (
        {
            type: 'checkbox',
            name: 'menu',
            message: "What would you like to do? (Choose only 1 or press ctrl + c to quit and save)",
